...
- **Hex bridge defense puzzle where Red already wins**: The board state had a complete Red path from row 0 to row 10 before the player moves, making the "defend the bridge" puzzle nonsensical. Fix: remove a direct connection so the bridge is actually needed.
- **Blokus tactics with too many solutions (not real tactics)**: Small pieces (monominos, dominoes) on open boards have dozens of valid placements. Real tactics require: (a) large pieces (5-cell polyominoes), (b) constrained boards with 2-3 pieces already placed per color, (c) only 1-3 valid first placements verified by exhaustive engine search. Use `generate-blocks-tactics.mjs` to find positions where most placements fail.
- **Blokus opponent piece overlapping player piece**: Opponent pieces defined in JSON are placed regardless of where the player put their first piece. Fix: check for collision before placing opponent pieces, and design opponent positions that cannot conflict with any valid player placement.
